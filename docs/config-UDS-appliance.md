...
Сервис-провайдер отвечает за предоставление IP-сервисов. Сервисы, предоставляемые платформой, могут быть виртуальными рабочими столами по требованию или приложениями, предоставляемыми платформой виртуализации или постоянными физическими/виртуальными рабочими столами, назначенными конкретным пользователям путем назначения IP-адресов.
...
![рис 4](./images/uds-4.png)
 
 **Рис. 4.**

 ## Настройка платформы VDI на базе oVirt или RHEV
 
 В данном разделе описано развертывание платформы VDI через виртуальную инфраструктуру oVirt или RedHat Enterprise Virtualization (RHEV).
...
![рис 6](./images/uds-6.png)

 **Рис. 6.**
 
 2.	Дополнительные, как показано на Рис. 7:
...
![рис 8](./images/uds-8.png)

**Рис. 8**

С помощью кнопки «Enter Maintenance Mode» можно приостановить все операции, запущенные платформой для данного сервис-провайдера. Рекомендуется поставить провайдер в режим обслуживания в случае потери связи или его остановки для обслуживания.
...
![рис 11](./images/uds-11.png)

 **Рис. 11**

После сохранения этой конфигурации будет готов действующий «oVirt Linked Clone» на платформе oVirt, как показано на Рис. 12. Можно зарегистрировать необходимое количество «OVirt Linked Clone» на платформе UDS. 

![рис 12](./images/uds-12.png)
...
![карт](./images/uds-17.png)
 
     **Рис. 17**
     
     Для поиска укажите имя группы или его часть и нажмите “Search”.
     
![карт](./images/uds-18.png)

      **Рис. 18**
      
      Выберите нужную группу в результатах поиска и нажмите “Accept”.

![карт](./images/uds-19.png)
       
       **Рис. 19**
       
       Пользователи, состоящие в добавленных в аутентификатор группах, будут автоматически добавлены в систему при первом входе. Исключение составляют аутентификаторы “Internal Database” и “IP Authenticator”, для которых пользователи добавляются только вручную.
       Для добавления пользователя выберите нужный аутентификатор в списке, перейдите на вкладку “Users” и нажмите “New”.

![карт](./images/uds-20.png)
...
![карт](./images/uds-24.png)
 
 **Рис. 24**
 ## Настройка транспортов подключения
 
 Для подключения к виртуальным рабочим столам и приложениям необходимо создать транспорт подключения. Транспорт - это небольшие приложения, которые отвечают за предоставление доступа к развернутому сервису и запускаются на клиенте.
...
![карт](./images/uds-25.png)

  **Рис. 25**
  
![карт](./images/uds-26.png)
...
![карт](./images/uds-32.png)

         **Рис. 32**
         
         2. Параметры доступности:
...
![карт](./images/uds-33.png)

          **Рис. 33**
          
          Для предоставления пользователям доступа к сервису необходимо добавить соответствующую группу пользователей в созданный пул сервисов. Группы пользователей создаются в настройках аутентификатора. Для добавления группы нажмите “New” на вкладке “Groups” созданного пула сервисов, как показано на Рис. 34.

![карт](./images/uds-34.png)

           **Рис. 34**
           
           Выберите аутентификатор и группу из списка доступных для выбранного аутентификатора.

![карт](./images/uds-35.png)
