...
-rw-r--r--. 1 root root 57619 Oct 22 11:18 HVirt.zip
[root@host1 ~]# pwd
/root
...
Укажите ваш DNS сервер (10.1.64.248): 10.1.64.248
DNS: 10.1.64.248
...
Выполните команду `ansible-playbook /etc/ansible/make-prepare.yml` для того, чтобы подготовить /etc/hosts и диск с указанным guid к работе. **Внимание! Указанный диск будет полностью очишен!**
*failed в конце выполнения этой команды - это нормально*
```
...
PLAY RECAP **********************************************************************************************************************************************************************************************************************************
localhost                  : ok=1    changed=2    unreachable=0    failed=1
...
['http://resources.ovirt.org/pub/yum-repo/ovirt-release43.rpm']` and remove the loop. This feature will be removed in version 2.11. Deprecation warnings can be disabled by setting deprecation_warnings=False in ansible.cfg.
...
После завершения установки в драузере перейдите по адресу engine.<mydomain.ru>  
