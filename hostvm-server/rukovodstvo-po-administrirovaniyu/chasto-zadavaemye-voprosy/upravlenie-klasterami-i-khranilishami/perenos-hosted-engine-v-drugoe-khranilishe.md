---
description: >-
  Данное руководство описывает действия по миграции виртуальной машины HOSTVM
  Manager между доменами хранения.
---

# Перенос Hosted HOSTVM Manager в другое хранилище

Для переноса Hosted HOSTVM Manager необходимо выполнить резервное копирование существующей установки, и новую установку на целевое хранилище с восстановлением данных из резервной копии. Ключевые действия при переносе Hosted HOSTVM Manager:&#x20;

·         Резервное копирование Hosted HOSTVM Manager

·         Установка Hosted HOSTVM Manager на новое хранилище и восстановление данных из резервной копии

·         Обновление конфигурации хостов для работы с новой ВМ HOSTVM Manager

·         Удаление старого домена хранения Hosted HOSTVM Manager

**До начала переноса необходимо выполнить следующие условия:**

·         Переместить все диски виртуальных машин с хранилища, на котором расположена ВМ HOSTVM Manager (кроме дисков этой машины HOSTVM Manager). Выполнение процедуры переноса делает недоступным исходное хранилище, поэтому убедитесь, что оно не содержит дисков ВМ, иначе они будут потеряны.

·         Обеспечить разрешение имен HOSTVM Manager и хоста, с которого будет выполняться установка. Новый HOSTVM Manager должен иметь такой же FQDN, как и исходный.
...
·         Подготовить новое хранилище для развертывания Hosted HOSTVM Manager, удовлетворяющее системным требованиям

### Резервное копирование исходной ВМ HOSTVM Manager

Подключитесь по SSH на хост, на котором запущена ВМ HOSTVM Manager. Выполните:
...
Подключитесь по SSH к ВМ HOSTVM Manager. Остановите службу:
...
Файл архива (в примере /root/hvm-backup.tgz ) содержит резервную копию конфигурационных файлов и баз данных менеджера виртуализации. Этот файл должен быть скопирован с машины HOSTVM Manager на внешнее хранилище, для дальнейшего использования, например:

```
# scp -p /root/hvm-backup.tgz /root/backup.log storage.example.com:/backup/
```

На хосте, на котором запущена ВМ HOSTVM Manager, выполните команду для ее выключения:
...
1. Загрузите файл резервной копии на хост, с которого будет выполнятся новая установка Hosted HOSTVM Manager.
...
5. Запустите новую установку Hosted HOSTVM Manager, указав местоположение файла резервной копии:
...
10. Укажите, следует ли добавить записи для HOSTVM Manager и хоста в файл /etc/hosts виртуальной машины. Вы должны убедиться, что эти имена будут разрешаться в IP адреса.&#x20;
...
17. После того как развертывание Hosted HOSTVM Manager будет закончено, портал администрирования будет доступен по прежнему адресу.
