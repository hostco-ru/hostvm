...
{% hint style="warning" %}
После удаления из кластера хоста с ролью **Self-Hosted** метаданные об этом хосте не удаляются автоматически с остальных хостов, также имеющих роль **Self-Hosted**. В результате при выполнении команды `hosted-engine --vm-status` удалённый хост может отображаться как недоступный.

Такой хост исчезнет из списка приблизительно через неделю — после планового обновления метаданных.

Для принудительного удаления метаданных хоста выполните действия, описанные в статье **«**[Очистка метаданных на хостах кластера с ролью Self-Hosted](../../chasto-zadavaemye-voprosy/ustanovka-i-nastroika-khostov/ochistka-metadannykh-na-khostakh-klastera-s-rolyu-self-hosted.md)**»**.
{% endhint %}
