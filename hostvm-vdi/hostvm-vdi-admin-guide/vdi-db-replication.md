...
<summary>Если используются внутренние БД из апплаенса брокера HOSTVM VDI</summary>
...
Перед настройкой реплики остановить сервисы первого брокера (_**10.0.0.1**_):
...
На первом (_master_) сервере БД  (_**10.0.0.1 - если внешняя база данных не используется, 10.0.0.3 - если используется)**_ отредактировать `/etc/mysql/mariadb.conf.d/50-server.cnf`

В параметре `bind-address` указать:\
IP адрес адрес (_**10.0.0.1 - если внешняя база данных не используется, 10.0.0.3 - если используется)**_.
