...
<figure><img src="https://raw.githubusercontent.com/hostco-ru/hostvm/master/.gitbook/assets/rds-sp-1.png" alt=""><figcaption></figcaption></figure>

Выполните настройку нового провайдера, задав параметры:

**Имя:** - наименование создаваемого провайдера для отображения в системе;

**Список серверов:** список серверов Microsoft RDS, используемых для терминального доступа или публикации приложений (роль RDSH). Задайте IP адреса серверов, разделенные запятыми, и нажмите "Добавить":
...
Балансировка выполняется одинаково как при подключении к сервисам терминальных сессий, так и к сервисам виртуальных приложений.

При выборе сервера для подключения используется специальный алгоритм на основе хэша имени пользователя. При условии неизменности списка серверов и их доступности для подключения, конкретному пользователю всегда будет выбран один и тот же сервер. В случае изменения списка (добавления или удаления серверов) результат может быть другим.

Алгоритм выбора не учитывает текущее состояние сессий на серверах.

IP-адрес выбранного сервера фиксируется в «назначенном сервисе» пользователя при первом обращении к сервис-пулу. При всех последующих подключениях (независимо, была ли сессия завершена путем закрытия соединения или выхода из системы) брокер будет направлять пользователя на выбранный сервер, пока он остается доступным для подключений.\
Поведение на стороне терминального сервера при повторном подключении (подключится ли пользователь к существующей сессии или будет создана новая) зависит от его настроек и используемого протокола.

Повторно процесс выбора сервера запускается в следующих случаях:

1. если для пользователя отсутствует «назначенный сервис» в пуле (например после ручного сброса сервиса администратором или самим пользователем);
2. если выбранный ранее сервер в момент подключения недоступен.
...
<figure><img src="https://raw.githubusercontent.com/hostco-ru/hostvm/master/.gitbook/assets/rds-sp-3.png" alt=""><figcaption></figcaption></figure>

Для внесения изменений в конфигурацию провайдера отметьте его и нажмите "Редактировать".

<figure><img src="https://raw.githubusercontent.com/hostco-ru/hostvm/master/.gitbook/assets/rds-sp-4.png" alt=""><figcaption></figcaption></figure>
...
<figure><img src="https://raw.githubusercontent.com/hostco-ru/hostvm/master/.gitbook/assets/rds-sp-5.png" alt=""><figcaption></figcaption></figure>
...
<figure><img src="https://raw.githubusercontent.com/hostco-ru/hostvm/master/.gitbook/assets/rds-sp-6.png" alt=""><figcaption></figcaption></figure>
...
<figure><img src="https://raw.githubusercontent.com/hostco-ru/hostvm/master/.gitbook/assets/rds-actor-2.png" alt=""><figcaption></figcaption></figure>
