...
# Виртуальный апплаенс

## Импорт виртуальной машины <a href="#vm-import" id="vm-import"></a>

* Скачайте образ машины `HOSTVM-VDI/Stable/<номер версии>/hostvm-gw<номер версии>-<номер сборки>.tar.bz2` из каталога загрузок HOSTVM.
* Загрузите данный файл на один из хостов виртуализации HOSTVM с помощью scp или других средств.
* Распакуйте командой: `tar xjvf hostvm-gw<номер версии>-<номер сборки>.tar.bz2`
...
В процессе установки будет сгенерирован самоподписанный сертификат для туннелера. Вы можете заменить его на собственный: [Сертификат для HTML5 подключений](virtual-appliance.md#html5-certificate). При наличии сертификата в системе (например при настройке существующей инсталляции) вы сможете сохранить старый сертификат либо сгенерировать новый.
...
{% hint style="info" %}
ВАЖНО: Если используется балансировщик, в настройках транспорта укажите порт, заданный на балансировщике. Порт туннелера по умолчанию - 443.
{% endhint %}
...
## Версия 3.0 (legacy) <a href="#id-3.0" id="id-3.0"></a>
