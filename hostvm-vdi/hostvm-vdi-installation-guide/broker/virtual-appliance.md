...
Примеры конфигурации для внешних СУБД:

```
Пример конфигурации для СУБД MariaDB (MySQL): 
Выберите используемую СУБД:
1) MariaDB (MySQL) - внешний сервер БД  4) PostgreSQL - локальная БД
2) PostgreSQL - внешний сервер БД       5) Выход
3) MariaDB (MySQL) - локальная БД
Введите номер опции: 1
Укажите IP адрес сервера БД: 10.1.99.51
...
Пример конфигурации для СУБД PostgreSQL:
Выберите используемую СУБД:
1) MariaDB (MySQL) - внешний сервер БД  4) PostgreSQL - локальная БД
2) PostgreSQL - внешний сервер БД       5) Выход
3) MariaDB (MySQL) - локальная БД
Введите номер опции: 2
Укажите IP адрес сервера БД: 10.1.99.52
Укажите порт сервера БД [5432]:
Укажите имя базы данных [udsdb]:
Укажите пользователя для подключения к серверу БД [udsdbadm]:
Введите пароль пользователя БД:
Введите (еще раз) пароль пользователя БД:
...
> Начиная с версии брокера 3.6-20250314 и выше доступна возможность [установки пароля администратора из консоли](../../troubleshooting-guide/faq/reset-admin-password.md#cli) без необходимости доступа к веб-интерфейсу.
...
### Настройка для работы с СУБД PostgreSQL (устарело) <a href="#postgresql-3.6-20250314" id="postgresql-3.6-20250314"></a>

{% hint style="info" %}
Инструкция для версий 3.6-20250314 и ниже.

При установке версии брокера 3.6-20250704 и выше пропустите этот шаг. Для настройки используйте [мастер установки HOSTVM VDI](virtual-appliance.md#setup-wizard).
{% endhint %}
